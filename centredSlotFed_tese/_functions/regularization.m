...
double precision; format long;
    ...
    fk = (2.*(1:n)+1 )./((1:n).*((1:n)+1 ));  I   = eye(N*n);     i0 = 1:N  ;     Ii = diag(ones(1,(N-1)),-1);
    ...
    for n_=1:n
        ...
        const = (2*n_+1)/(2*n_*(n_+1)*k___s*r__s);
        
        CKan_ = K3Tan - K1Tan/Z1Tan*Z3Tan;   A1s =  K1__s(1,1,n_)*const*d1;       CKan(:,:,n_) = CKan_;
        CZan_ = Z3Dan + Z1Ean/Z1Tan*Z3Tan;   A3s =  K3__s(1,1,n_)*const*d0;       Gan(:,:,n_) = I_N - fk(n_)/fz(n_)*(Omg_a\CZan_)/CKan_;

        CZbn_ = Z3Tbn - Z1Tbn/K1Tbn*K3Tbn;   B1s =  Z1__s(1,1,n_)*const*d1;       CZbn(:,:,n_) = CZbn_;
        CKbn_ = K3Dbn + K1Ebn/K1Tbn*K3Tbn;   B3s =  Z3__s(1,1,n_)*const*d0;       Gbn(:,:,n_) = I_N - fz(n_)/fk(n_)*(Omg_b\CKbn_)/CZbn_;
