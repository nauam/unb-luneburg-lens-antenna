function an_erroRelativo
    ...
    Xa = zeros((n+1)*N,n+1); Xb = zeros((n+1)*N,n+1); 
    erro_a = zeros(n,length(k0r)); erro_b = zeros(n,length(k0r));  erro_max = zeros(n,length(k0r)); 
    ...
    for k0r_ = k0r
        ...
        for n_ = 1:n+1
            [Xa(1:n_*N,n_),Xb(1:n_*N,n_),~] = regularization(n_,N,k0,f_s,r__s,r,th_r,ep_r,mu_r);
        ...
        for n_ = 1:n
            ...
            erro_max(n_,i_) = max([erro_a(n_,i_) erro_b(n_,i_)]);
        end
    ...
    %Gráfico
    figure(1)
    semilogy(1:n,erro_max,'Linewidth',2);  hold on
